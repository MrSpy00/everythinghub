...
export function RegexTesterClient() {
  ...
  const [text, setText] = useState<string>(
    "İletişim için destek@everythinghub.com veya mrspy00@github.com adreslerine e-posta gönderebilirsiniz."
  );
  ...
  try {
    ...
    error = err instanceof Error ? err.message : "Geçersiz Regex kalıbı";
  }
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs sm:text-sm text-[var(--hub-text-muted)]">
              Düzenli ifadelerinizi (Regular Expressions) canlı metinler üzerinde anlık test edin.
            </p>
          </div>
        </div>
      </div>
      ...
      <NeonBorder color="#3b82f6" rounded={24} glow={60}>
        <div className="rounded-[22px] bg-[var(--hub-surface)]/95 p-6 backdrop-blur-3xl shadow-2xl space-y-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
            <div className="sm:col-span-3">
              <label className="text-xs font-bold text-blue-300 uppercase tracking-wider block mb-2 flex items-center gap-1.5">
                <Sparkles className="h-3.5 w-3.5 text-blue-400" /> Regex Pattern
              </label>
            ...
            <div>
              <label className="text-xs font-bold text-[var(--hub-text-muted)] uppercase tracking-wider block mb-2">Flags</label>
              <input
                type="text"
                value={flags}
          ...
          <div>
            <label className="text-xs font-bold text-white mb-2 block">Test Edilecek Metin</label>
            <textarea
              value={text}
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-emerald-400 flex items-center gap-1.5">
                <CheckCircle2 className="h-4 w-4" /> Eşleşen Sonuçlar ({matches.length})
              </span>
            </div>
            {matches.length > 0 ? (
