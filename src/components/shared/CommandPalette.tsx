...
export function CommandPalette({ isOpen, onClose }: CommandPaletteProps) {
  ...
  const filteredTools = tools.filter((tool) => {
    ...
    const categoryMatch = (CATEGORY_LABELS[tool.category] || tool.category).toLowerCase().includes(q);
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4">
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-white/10 bg-[#0d0e12]/95 backdrop-blur-3xl shadow-2xl shadow-indigo-500/10"
        >
          {/* Header Input */}
          <div className="flex items-center gap-3 border-b border-white/10 px-4 py-3.5">
            ...
            <input
              ...
              placeholder="Araç ismi, kategori veya etiket ara... (Örn: JSON, Spotify, Döviz, Cron)"
            ...
            <kbd className="hidden sm:inline-flex items-center gap-1 rounded bg-white/10 px-2 py-0.5 text-[10px] font-semibold text-zinc-400 border border-white/10">
          ...
          <div className="max-h-[60vh] overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-white/10">
            {filteredTools.length === 0 ? (
              <div className="py-12 text-center text-zinc-500 text-sm">
                Aramanızla eşleşen hiçbir araç bulunamadı.
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="hidden sm:inline-block rounded-md bg-white/5 px-2 py-0.5 text-[10px] font-medium text-zinc-400 border border-white/5">
          ...
          {/* Footer status bar */}
          <div className="flex items-center justify-between border-t border-white/10 px-4 py-2 text-[11px] text-zinc-500 bg-white/[0.02]">
            <span className="flex items-center gap-1.5">
              <Sparkles className="h-3 w-3 text-indigo-400" />
              EverythingHub Dijital Araçlar Stüdyosu
            </span>
