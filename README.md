...
<p align="center">
  ...
  <em>Modern, ultra-fast, zero-login, zero-auth, privacy-first all-in-one digital tools studio.</em>
</p>
...
## Canlı Araç Kataloğu (38 Aktif Stüdyo Aracı)

### Geliştirici & Ağ Stüdyoları
1. **Canlı API Test Konsolu & Açık API Kataloğu (`/tools/api-playground`):** 150+ ücretsiz ve sıfır-auth açık API'yi canlı test edin, özel HTTP GET/POST istekleri gönderin, yanıt süresi ve JSON yüklerini inceleyin.
2. **İstemci Taraflı JWT Debugger & Token Çözücü (`/tools/jwt-debugger`):** JSON Web Token (JWT) başlık, payload ve imza verilerini %100 tarayıcı tarafında çözün. Canlı süre geri sayımı ve claim ayrıştırma.
3. **Kriptografik Hash, HMAC & UUID Laboratuvarı (`/tools/crypto-hash-studio`):** SHA-256, SHA-512, MD5, HMAC ve UUID v4/v7/NanoID üretimini Web Crypto API ile anında gerçekleştirin.
4. **SQL'den TypeScript, Zod & Prisma Dönüştürücü (`/tools/sql-to-types`):** SQL CREATE TABLE tablolarını anında TypeScript interface, Zod şeması, Prisma modeli, Go struct ve Python Pydantic kodlarına dönüştürün.
5. **cURL'den Çoklu Dil Kod Üreticisi (`/tools/curl-to-code`):** cURL komutlarını anında JavaScript fetch, Axios, Python requests, Go ve PHP kodlarına dönüştürün.
6. **Geliştiriciler İçin Akıllı Mock Veri Üreteci (`/tools/mock-data-generator`):** Testler ve prototipler için Türkçe ve uluslararası sahte kimlikler, adresler, telefon numaraları ve JSON/CSV mock verileri üretin.
7. **IP & Ağ Latency Analizörü (`/tools/ip-network-info`):** IP adresinizi, coğrafi konumunuzu, ISP sağlayıcınızı ve CDN ping sürelerinizi 4 bağımsız kaynaktan canlı analiz edin.
8. **DNS Kayıtları & DoH Sorgulayıcı (`/tools/dns-lookup-tool`):** A, AAAA, MX, TXT ve CNAME DNS kayıtlarını Cloudflare & Google DoH güvenli altyapısıyla anında sorgulayın.
9. **JSON Formatlayıcı & Validator (`/tools/json-formatter`):** JSON beautify, minify, sözdizimi doğrulama ve ağaç görünümü.
10. **Base64 Kodlayıcı & Çözücü (`/tools/base64-encoder`):** UTF-8 destekli güvenli Base64 encode ve decode.
11. **İnteraktif Regex Tester (`/tools/regex-tester`):** Düzenli ifadeleri canlı metinler üzerinde test etme, bayrak (flag) yönetimi ve eşleşmeleri anlık listeleme.
12. **Cron İfadesi Üreteci & Açıklayıcı (`/tools/cron-expression-studio`):** Linux crontab zamanlamalarını görsel olarak üretin ve Türkçe insani açıklamasını görün.

### Tasarım & Vektör Stüdyoları
13. **QR Kod Stüdyosu Pro (`/tools/qr-code-studio`):** 12 veri tipi (URL, Metin, Wi-Fi, vCard 3.0, E-Posta, SMS, WhatsApp, Telefon, Kripto Cüzdanlar, Harita Konumu, iCal Takvim, Sosyal Medya), degrade renkler, özel gövde ve köşe desenleri, merkez marka logo rozetleri, "SCAN ME" CTA çerçeveleri, 4096px baskıya hazır PNG ve saf vektörel SVG dışa aktarma.
14. **Vektörel Barkod Üreteci (`/tools/barcode-generator`):** EAN-13, UPC-A, Code 128 ve Code 39 barkod standartlarında baskıya hazır vektörel SVG ve yüksek çözünürlüklü PNG barkodlar üretin.
15. **Site Favicon & Logo İndirici (`/tools/favicon-extractor`):** Herhangi bir web sitesinin HD favicon, Apple touch icon ve SVG logosunu anında indirin.
16. **Renk Paleti & Resimden Renk Çıkarıcı (`/tools/color-picker`):** Yüklediğiniz görselden dominant renk paletini çıkarın, HEX, RGB ve HSL formatında kopyalayın.
17. **CSS & Tailwind Gradient Üretici (`/tools/css-gradient-generator`):** Çok renkli doğrusal ve dairesel CSS/Tailwind gradientler oluşturun, tek tıkla CSS kodunu alın.

### Görsel Stüdyoları
18. **EXIF Metaveri İnceleyici & Gizlilik Temizleyici (`/tools/exif-purger`):** Fotoğraflarınızdaki GPS konum koordinatlarını, kamera modelini inceleyin ve paylaşmadan önce %100 temizleyin.
19. **Görsel Sıkıştırıcı (`/tools/image-compressor`):** Cihazınızda sıfır veri transferi ile %90'a varan oranda kayıpsız görsel sıkıştırma ve WebP optimizasyonu.
20. **Görsel Format Dönüştürücü (`/tools/image-converter`):** PNG, JPEG, WebP ve AVIF formatları arasında anında kalite kaybı olmadan dönüştürme yapın.

### Ses & Müzik Stüdyoları
21. **Web Audio Osiloskop & Spektrum Analizörü (`/tools/audio-spectrum-studio`):** Mikrofon veya ses dalgalarını Web Audio API ile gerçek zamanlı osiloskop ve FFT frekans spektrumu olarak analiz edin.
22. **Hassas BPM Tapper & Akıllı Metronom (`/tools/bpm-tapper`):** Klavyeden veya dokunarak anlık BPM hesaplayın, tutarlılık sapmasını görün ve Web Audio milisaniye zamanlayıcılı metronom ile ritim tutun.
23. **Spotify Playlist Analizör (`/tools/spotify-playlist-analyzer`):** Chosic & Artist.tools seviyesinde Sonic DNA radarı, bot & fraud tespiti, tür dağılımı, BPM/Key çarkı ve HD kapak stüdyosu.
24. **Spotify Profil & Sanatçı Analizör (`/tools/spotify-profile-analyzer`):** Küratör ve sanatçı profillerini inceleyin, takipçi gücünü, diskografiyi, en popüler şarkıları ve HD avatarları çıkarın.

### Video Stüdyoları
25. **YouTube Playlist Analyzer (`/tools/yt-playlist-length`):** Playlist süresini hesapla, farklı hızlarda izleme sürelerini gör, HD thumbnail ve CSV dışa aktar.
26. **YouTube Thumbnail İndirici (`/tools/yt-thumbnail-downloader`):** Herhangi bir YouTube videosunun HD, Full HD ve 4K kapak görsellerini anında ücretsiz indirin.
27. **YouTube Zaman Damgası Üretici (`/tools/yt-timestamp-generator`):** Videolar için saniye/dakika bazlı başlama bağlantıları (timestamp link) ve açıklama listeleri üretin.

### Metin, Dil & Kültür Stüdyoları
28. **İngilizce Akıllı Sözlük, Telaffuz & Kafiye Motoru (`/tools/smart-dictionary`):** Web Speech API ile Amerikan & İngiliz aksanlı telaffuz, hız kontrolü, zıt anlamlılar (antonyms), kafiye motoru ve seçkin akademik kelime koleksiyonları.
29. **Açık Kitaplık & ISBN Arama Motoru (`/tools/book-isbn-finder`):** 20M+ kitap, yazar ve ISBN numarası üzerinden kitap kapağı, yayıncı, sayfa sayısı ve basım yılı bilgilerini arayın.
30. **İnteraktif Bilgi Yarışması & Trivia Arena (`/tools/trivia-quiz-arena`):** Bilim, teknoloji, tarih ve genel kültür alanlarında binlerce soruyla bilginizi test edin.
31. **Kelime ve Metin Sayacı (`/tools/word-counter`):** Metninizin kelime, karakter, cümle, paragraf sayısını ve tahmini okuma süresini anlık hesaplayın.
32. **Büyük/Küçük Harf & Metin Dönüştürücü (`/tools/case-converter`):** camelCase, snake_case, kebab-case, Title Case ve UPPERCASE dönüşümleri.
33. **Markdown Studio & Canlı Önizleme (`/tools/markdown-studio`):** Markdown dokümanlarınızı canlı önizleyin, metrikleri görün, HTML veya .md olarak indirin.

### Hesaplama, Coğrafya & Finans Stüdyoları
34. **Canlı Hava Durumu & Hava Kalitesi Radarı (`/tools/weather-air-quality`):** Anında IP/GPS konum tespiti, son konum hafızası, 7 günlük saatlik tahminler, UV indeksi ve PM2.5/PM10 hava kalitesi indeksi (AQI).
35. **Dünya Ülkeleri & Coğrafya Karşılaştırma Stüdyosu (`/tools/world-countries-explorer`):** REST Countries ile 250+ dünya ülkesinin bayrakları, nüfusu, başkenti, dilleri ve para birimlerini keşfedin ve karşılaştırın.
36. **Canlı Döviz & Kripto Dönüştürücü (`/tools/currency-exchange-converter`):** 160+ itibari para birimi ve top 25 kripto varlık arasında Avrupa Merkez Bankası (ECB) verileriyle kusursuz giriş ve canlı dönüşüm.
37. **Çoklu Birim Dönüştürücü (`/tools/unit-converter`):** Uzunluk, kütle, sıcaklık, veri depolama ve hız birimlerini hassas dönüştürün.
38. **Yüzde & İndirim Hesaplayıcı (`/tools/percentage-calculator`):** Yüzde artış/azalış, oran hesaplama, indirimli fiyat ve KDV tutarlarını anında hesaplayın.
...
Instead of generic dashboard templates, EverythingHub delivers a **creative studio aesthetic** featuring GPU-accelerated WebGL chromatic dotted waves, fluid magnetic follower cursor (`UserCursor`) with Originkit velocity physics and tilt, liquid glassmorphism, soft ambient Gaussian diffusion, `Ctrl+K` Command Palette, and intelligent circular scroll-to-top tracking.
...
13. **QR Code Studio Pro (`/tools/qr-code-studio`):** 12 data types (URL, Text, Wi-Fi, vCard 3.0, Email, SMS, WhatsApp, Phone, Crypto, Location, iCal Event, Social Links), custom gradients, center logo badges, "SCAN ME" CTA frames, 4096px print PNG and scalable SVG export.
...
28. **Smart English Dictionary & Rhyme Engine (`/tools/smart-dictionary`):** Web Speech API pronunciation with US/UK accents, speed control, antonyms, rhyming engine, and curated GRE/Tech vocabulary collections.
...
34. **Live Weather & Air Quality Radar (`/tools/weather-air-quality`):** Instant IP/GPS location detection with persistent memory, 7-day hourly forecasts, UV index, and PM2.5/PM10 AQI.
...
36. **Live Currency & Crypto Converter (`/tools/currency-exchange-converter`):** Seamless input handling for 160+ fiat currencies and top 25 crypto assets with ECB live rates.
